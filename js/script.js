...
const seleccionarPersonaje = (event) => {
    ...
    let personajes = menu__config.querySelectorAll("IMG")
    ...
    if(event.target.tagName == "BUTTON"){
        ...
        for(let i = 0; i < personajes.length; i++){
            ...
            if(personajes[i].className == "seleccionado"){
                
                let personaje = personajes[i].src.split("/")[10];    
             
                cargarJuego( personaje.split("_")[0] )
            }else{
                btn__jugar.style.backgroundColor = "red"
            }
...
const turnoEnemigo = (defensa) => {
    ...
    caja__texto.innerHTML= "El enemigo falla el ataque"
            ...
            //El ataca al jugador con un crítico
        if(random < 3){
...
menu__config.addEventListener("click", seleccionarPersonaje)
